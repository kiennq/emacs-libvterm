...
emacs_value Fvterm_conpty_init(emacs_env *env, ptrdiff_t nargs,
  ...
  if (term->temp_arena != NULL) {
    shell_cmd = (char *)arena_alloc(term->temp_arena, shell_len);
  } else {
    shell_cmd = (char *)malloc(shell_len);
  }
  if (!shell_cmd) {
    CONPTY_LOG("Fvterm_conpty_init: alloc for shell_cmd failed\n");
    return Qnil;
  }
  ...
  if (width <= 0 || height <= 0) {
    ...
    if (term->temp_arena == NULL) free(shell_cmd);
    return Qnil;
  }
  ...
  if (term->temp_arena != NULL) {
    si.lpAttributeList = (LPPROC_THREAD_ATTRIBUTE_LIST)arena_alloc(
        term->temp_arena, attr_size);
  } else {
  ...
  if (!InitializeProcThreadAttributeList(si.lpAttributeList, 1, 0,
    ...
    CONPTY_LOG("Fvterm_conpty_init: InitializeProcThreadAttributeList or "
               ...
               GetLastError());
    if (term->temp_arena == NULL) free(si.lpAttributeList);
    conpty_cleanup(term);
    if (term->temp_arena == NULL) free(shell_cmd);
    return Qnil;
  }
  ...
  wchar_t *wshell;
  if (term->temp_arena != NULL) {
    wshell = (wchar_t *)arena_alloc(term->temp_arena, wlen * sizeof(wchar_t));
  } else {
    wshell = (wchar_t *)malloc(wlen * sizeof(wchar_t));
  }
  if (!wshell) {
    CONPTY_LOG("Fvterm_conpty_init: alloc for wshell failed\n");
  ...
  if (env->is_not_nil(env, dir_val)) {
    ...
    if (env->is_not_nil(env, expanded)) {
      ...
      char *directory;
      if (term->temp_arena != NULL) {
        directory = (char *)arena_alloc(term->temp_arena, dir_len);
      } else {
      ...
      if (directory) {
        ...
        if (term->temp_arena == NULL) free(directory);
      }
    }
  }
  ...
  if (term->temp_arena == NULL) free(wshell);
  if (term->temp_arena == NULL) free(wdirectory);
  DeleteProcThreadAttributeList(si.lpAttributeList);
  if (term->temp_arena == NULL) free(si.lpAttributeList);
...
emacs_value Fvterm_conpty_write(emacs_env *env, ptrdiff_t nargs,
  ...
  char *bytes;
  if (term->temp_arena != NULL) {
    bytes = (char *)arena_alloc(term->temp_arena, len);
  } else {
    bytes = (char *)malloc(len);
  }
  if (!bytes) {
    CONPTY_LOG("Fvterm_conpty_write: alloc failed\n");
    return Qnil;
  }
  ...
  if (term->temp_arena == NULL) free(bytes);
